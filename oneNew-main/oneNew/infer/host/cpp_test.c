...
int32_t submit_task(const char* in_buf, const char* out_buf){
    ...
    // void* handle = dlopen("./libadd1.so", RTLD_LAZY);

    // printf("hhtttt ese\n");
    // int* (*add_c)(const char* , const char* ) = (int* (*)())dlsym(handle, "add_cuda_liu");

    // void* handle = dlopen("./libadd_dev.so", RTLD_LAZY);

    // printf("hhtttt ese\n");
    // int* (*add_c)(const char* , const char* ) = (int* (*)())dlsym(handle, "add_liu");
