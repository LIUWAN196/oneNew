...
# 这个函数是打印有效的帮助信息，帮助使用者使用本个 .sh。其中 $0 表示的就是本个 .sh 的名字； -e 就是让 \ 被视作转义字符
function usage_() {
    echo -e "Usage: [build_path] [onnx_path] [one_path] [optional params: yes]"
    echo -e "if the second input is \"yes\", it will show_init_value"
    echo -e "the input params num is $#"
}
...
function main() {
    ...
    # 如果有 build, 则将这个目录删除并 mkdir 这个目录
    if [ -d $1 ]; then
        rm -rf $1
    fi
    mkdir $1
