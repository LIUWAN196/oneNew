...
//        params_vec.push_back(params);
    ...
    {
        // fill op type and op name
        op_type = (char*)(&(this->log_cfg));
        op_name = (char*)((int64_t)&(this->log_cfg) + OP_TYPE_LEN);

        BASE_CONFIG_S* base_cfg = (BASE_CONFIG_S*)(&(this->log_cfg));
        int32_t in_operand_num = base_cfg->in_operand_num;
        int32_t out_operand_num = base_cfg->out_operand_num;

        for (int in_i = 0; in_i < in_operand_num; ++in_i) {
            std::string in_operand(this->log_cfg.op_base_cfg.in_operand_name[in_i]);
            this->in_operands.push_back(in_operand);
        }

        for (int out_i = 0; out_i < out_operand_num; ++out_i) {
            std::string out_operand(this->log_cfg.op_base_cfg.out_operand_name[out_i]);
...
OP_REGISTER_GLOBAL(Log, Log::create_instance, sizeof(LOG_CONFIG_S));
