...
{
public:
    DIV_CONFIG_S div_cfg;

    // 有可能 div 的第二个输入数据是 init 的
...
//        printf("new a Div\n");
    ...
    {
        // new Div op
        std::shared_ptr<Div> div_ptr = std::make_shared<Div>();
    ...
    virtual int calc_out_operand_shape(std::unordered_map<std::string, OPERAND_S> &operand_stu_map) override {
        ...
        if (strcmp(in->operand_name, "/model.10/attn/MatDiv_output_0") == 0) {
            int a = 101;
        }
...
//        params_vec.push_back(params);
    ...
    {
        // fill op type and op name
        op_type = (char*)(&(this->div_cfg));
        op_name = (char*)((int64_t)&(this->div_cfg) + OP_TYPE_LEN);

        if (strcmp(op_name, "/model.10/attn/Div") == 0) {
            int a = 101;
        }

        BASE_CONFIG_S* base_cfg = (BASE_CONFIG_S*)(&(this->div_cfg));
        int32_t in_operand_num = base_cfg->in_operand_num;
        int32_t out_operand_num = base_cfg->out_operand_num;

        for (int in_i = 0; in_i < in_operand_num; ++in_i) {
            std::string in_operand(this->div_cfg.op_base_cfg.in_operand_name[in_i]);
            this->in_operands.push_back(in_operand);
        }

        for (int out_i = 0; out_i < out_operand_num; ++out_i) {
            std::string out_operand(this->div_cfg.op_base_cfg.out_operand_name[out_i]);
...
//                std::cout << "the init operand is weight of " << this->op_type << "op." << std::endl;
    ...
    virtual double get_computation() override {
        ...
        return (double)(2 * out_elem_size);
    };

};

OP_REGISTER_GLOBAL(Div, Div::create_instance, sizeof(DIV_CONFIG_S));
