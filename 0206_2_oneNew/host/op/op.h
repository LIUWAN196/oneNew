...
//        snprintf(op_lib_path, sizeof(op_lib_path), "%s%s%s%s", OP_LIB_DIR, "/lib", op_lib_name.c_str(), ".so");
        snprintf(op_lib_path, sizeof(op_lib_path), "%s%s%s%s%s", OP_LIB_DIR, op_lib_name.c_str(), "/lib", op_lib_name.c_str(), ".so");
    ...
    virtual int forward(std::unordered_map<std::string, std::vector<int8_t>> &operand_buf_map, std::unordered_map<std::string, OPERAND_S> &operand_stu_map)
    {
//        std::cout << "start forward. " << std::endl;
        // step 1： set the operand desc
        for (int i = 0; i < this->in_operands.size(); ++i) {
            BUFFER_INFO_S in_buf;
            in_buf.addr = (int64_t)(&(operand_stu_map[this->in_operands[i]]));
            params_vec.push_back(in_buf);
        }

        for (int i = 0; i < this->out_operands.size(); ++i) {
            BUFFER_INFO_S out_buf;
            out_buf.addr = (int64_t)(&(operand_stu_map[this->out_operands[i]]));
            params_vec.push_back(out_buf);
        }
