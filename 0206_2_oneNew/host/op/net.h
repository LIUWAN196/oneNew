...
//                for (auto &operand_stu : operand_stu_vec) {
//                    std::string operand_name_ = std::string(operand_stu.operand_name);
//                    if (operand_name_ == out_operand_name) {
//                        operand_stu = io_ptr.get()->io_cfg.operand;
//
//                        int a = 101;
//                    }
//                }
...
int extractor::new_output_buf(){
    ...
    for (auto operand : net_ptr->operand_stu_map) {
        int32_t buf_size = operand_buf_size(&operand.second);
        if (buf_size == 0) {
