...
# 原始的 res18 结构
class relu_pool_conv(nn.Module):
    def __init__(self, num_classes=3) -> None:
        super(relu_pool_conv, self).__init__()
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=2, stride=2, padding=0, ceil_mode=False)
        self.conv1 = nn.Conv2d(3, 4, kernel_size=2, stride=2, padding=0, bias=False)
        self.avgpool = nn.AdaptiveAvgPool2d(output_size=(1, 1))
        self.fc = nn.Linear(512, num_classes)
...
if __name__ == '__main__':
    ...
    # tmp_model = liu_res18(ResBlock)
    #
    # tmp_model18 = liu_res18(ResBlock)

    input = torch.randn(1, 3, 8, 8)
    # tmp_model18 = relu_pool_conv()


    tmp_model_pt = './tmp_model.pt'
    tmp_2onnx_model = relu_pool_conv()
    ...
    # 保存模型的中间节点的 shape 信息
    tmp_onnx = "tmp_model_with_shape.onnx"
