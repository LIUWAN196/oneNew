...
int32_t fun_0(OPERAND_S ofmap_tensor, OPERAND_S small_tensor, OPERAND_S large_tensor, DIV_CONFIG_S* cfg);
int32_t fun_1(OPERAND_S ofmap_tensor, OPERAND_S small_tensor, OPERAND_S large_tensor, DIV_CONFIG_S* cfg);
int32_t fun_2(OPERAND_S ofmap_tensor, OPERAND_S small_tensor, OPERAND_S large_tensor, DIV_CONFIG_S* cfg);

int eval(BUFFER_INFO_S *params, BUFFER_INFO_S *inputs, BUFFER_INFO_S *outputs) {
//    show_dev_input(params);

    DIV_CONFIG_S *cfg = (DIV_CONFIG_S *) (params[0].addr);
    ...
    if (in0_elem_size < in1_elem_size) {
        LOG_ERR("sorry, we currently do not support a / b, where elem_size(a) < elem_size(b)");
    ...
    if (in0_elem_size == in1_elem_size) {
        ...
        // 此时两个 tensor 的维度长度都相同了, 做某些维度的广播除法，这些广播的维度可能相邻也可能不相邻
        ...
        if (dims_correct == FALSE) {
            LOG_ERR("div op 的 shape 不符合广播规则\n");
        }
...
int32_t fun_1(OPERAND_S ofmap_tensor, OPERAND_S small_tensor, OPERAND_S large_tensor, DIV_CONFIG_S* cfg) {
    ...
    int32_t ofmap_elem_size = operand_elem_size(&ofmap_tensor);
    float inv_small_val = 1.0f / small_ptr[0];
#pragma omp parallel for num_threads(8)
    for (int i = 0; i < ofmap_elem_size; ++i) {
        output_ptr[i] = large_ptr[i] * inv_small_val;
    }

    return 0;
}

int32_t fun_2_dim3(OPERAND_S ofmap_tensor, OPERAND_S small_tensor, OPERAND_S large_tensor, DIV_CONFIG_S* cfg);
int32_t fun_2_dim4(OPERAND_S ofmap_tensor, OPERAND_S small_tensor, OPERAND_S large_tensor, DIV_CONFIG_S* cfg);
int32_t fun_2_dim5(OPERAND_S ofmap_tensor, OPERAND_S small_tensor, OPERAND_S large_tensor, DIV_CONFIG_S* cfg);
int32_t fun_2_dim6(OPERAND_S ofmap_tensor, OPERAND_S small_tensor, OPERAND_S large_tensor, DIV_CONFIG_S* cfg);

int32_t fun_2(OPERAND_S ofmap_tensor, OPERAND_S small_tensor, OPERAND_S large_tensor, DIV_CONFIG_S* cfg) {

    int32_t ofmap_dims_num = ofmap_tensor.dim_num_of_shapes;
    if (ofmap_dims_num <= 2) {
        LOG_ERR("在 div op 中，输出的 shape 维度 <= 2\n");
        ...
        LOG_ERR("在 div op 中，输出的 shape 维度 > 6\n");
    }
