...
//    printf("\n yes this is device, the op type is %s, the op name is %s\n", cfg->op_type, cfg->op_name);
    ...
    int pad_left = cfg->pads[2];
    int pad_right = cfg->pads[4 + 2];
    int pad_top = cfg->pads[3];
    int pad_bottom = cfg->pads[4 + 3];
    ...
    for (int c_i = 0; c_i < dst_c; ++c_i) {
        for (int h_i = pad_top; h_i < dst_h - pad_bottom; ++h_i) {
            for (int w_i = pad_left; w_i < dst_w - pad_right; ++w_i) {
                ...
                = src_f32[c_i * src_h * src_w + (h_i - pad_top) * src_w + (w_i - pad_left)];
            }
