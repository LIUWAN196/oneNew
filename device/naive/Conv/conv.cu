...
//    const float input_sacle = 450.894;
    ...
    int8_t *input_s8_ptr = (int8_t *)malloc(in_n * in_c * in_h * in_w * sizeof(int8_t));
...
__global__ void calc_conv_as_gemm_gpu(float *input_ptr, float *weight_ptr, float *bias_ptr, float *output_ptr, float *arg_)
{
    int block_idx = blockIdx.x;
    int th_idx = threadIdx.x;

    // each thread store a arg
    ARGS_S arg_reg;
    arg_reg = *(ARGS_S*)arg_;
    ARGS_S * arg = &arg_reg;

    int out_elem_per_th_calc = (arg->GEMM_N + blockDim.x - 1) / blockDim.x;

    const int ic_stride = arg->in_h * arg->in_w;
    const int ih_stride = arg->in_w;
    const int weight_n_stride = arg->in_c * arg->FH * arg->FW;
    const int weight_c_stride = arg->FH * arg->FW;
    const int weight_h_stride = arg->FW;
    const int outc_stride = arg->out_h * arg->out_w;
    const int outh_stride = arg->out_w;
...
#pragma unroll
    for (int j = th_idx * out_elem_per_th_calc; j < (th_idx + 1) * out_elem_per_th_calc; j++) {
        ...
        int32_t j_res = j % (arg->out_h * arg->out_w);
        int32_t oh = j_res / arg->out_w;
        int32_t ow = j_res % arg->out_w;

        // calc single output's elem
        for (int k_i = 0; k_i < arg->FH * arg->FW; ++k_i){
            k_res = k_i;
            fh = k_res / arg->FW;
            fw = k_res % arg->FW;
#pragma unroll
            for (int ic_i = 0; ic_i < arg->in_c; ++ic_i){
                ic = ic_i;
                ih = oh * arg->stride_h + fh;
                iw = ow * arg->stride_w + fw;
                accumulator = accumulator + input_ptr[ic * ic_stride + ih * ih_stride + iw] *
                                            weight_ptr[block_idx * weight_n_stride + ic * weight_c_stride +
                                                       fh * weight_h_stride + fw];
            }
        }
        output_ptr[block_idx * outc_stride + oh * outh_stride + ow] = accumulator;
    }
...
//    printf("args.FH is %d, args.FW is %d\n", args.FH, args.FW);
//    printf("args.stride_h is %d, args.stride_w is %d\n", args.stride_h, args.stride_w);
//    printf("args.in_h is %d, args.in_w is %d\n",  args.in_h, args.in_w);
//    printf("out_h is %d, out_w is %d\n",  out_h, out_w);
//    printf("GEMM_K is %d, GEMM_N is %d\n",  GEMM_K, GEMM_N);
//    printf("kernel_h is %d, kernel_w is %d\n",  kernel_h, kernel_w);

    const int th_num = 256;
    dim3 grid(GEMM_M);
    dim3 block(th_num);
...
//    size_t bytes_c = M * N * sizeof(float);
    ...
    cudaMalloc((void **)&d_args, sizeof(ARGS_S));
    ...
    cudaMemcpy(d_args, &((float *)&args)[0], sizeof(ARGS_S), cudaMemcpyHostToDevice);

    calc_conv_as_gemm_gpu<<<grid, block>>>(d_in, d_weight, d_bias, d_out, d_args);
...
//    printf("22 the output is %f, %f, %f\n", output_ptr[0 + 112 * 112], output_ptr[1 + 112 * 112], output_ptr[2 + 112 * 112]);
//    printf("33 the output is %f, %f, %f\n", output_ptr[0 + 2 * 112 * 112], output_ptr[1 + 2 * 112 * 112], output_ptr[2 + 2 * 112 * 112]);
//
