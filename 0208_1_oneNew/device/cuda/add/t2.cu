...
{
          printf("this is add of gpu impl\n");
...
int add_cuda_liu(const char* in_buf, const char* out_buf)
// int main(void)
{
          printf("1111111 \n");
...
//   int N = 1<<25;
//   float *x, *y;
          printf("this is add of gpu manager hh\n");
...
//   std::cout << "Max hhhh error: " << maxError << std::endl;

//   // Free memory
//   cudaFree(x);
//   cudaFree(y);
  
  printf("end oooo this is add of gpu manager hh\n");

  return 0;
}


int add_liuaa(const char* in_buf, const char* out_buf){
    int32_t buf_size = 12;
    int i;
    printf("\n start of device add \n");
